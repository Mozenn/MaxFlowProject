...
let rec get_team tb tid = match tb with 
  | ([],_,_) -> raise (Table_error ("team " ^ string_of_int tid ^ " does not exist in this table.")) (*Raise error if team not found*)
...
(*adds all team nodes, game nodes, source and sink*)
...
(*build arcs between games and the two teams playing in it
  also build arcs between games and source node 
*)
...
(*
build arcs between teams & sink node, adjusting the capacity if negative flow 
*)
